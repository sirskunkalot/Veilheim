...
public static class ObjectDBExtensions
{
	internal static void Dump(this ObjectDB objectDB)
	{
		...
		Veilheim.Logger.LogWarning(consumablesTable);

		weaponsTable.Append("|}");
		Veilheim.Logger.LogWarning(weaponsTable);

		shieldsTable.Append("|}");
		Veilheim.Logger.LogWarning(shieldsTable);

		armorsTable.Append("|}");
		Veilheim.Logger.LogWarning(armorsTable);

		ammoTable.Append("|}");
		Veilheim.Logger.LogWarning(ammoTable);

		otherItemsTable.Append("|}");
		Veilheim.Logger.LogWarning(otherItemsTable);
	}
}